...
(function(){
  ...
  theme.cart = {
    ...
    _updateCart: function(params, key) {
      ...
      .then(function(cart) {
        ...
        // const contextBeforeColon = keyID[0];
        const size = document.querySelector(`.cart__item[data-key="${key}"] .dropbtn`);
        const sizeName = size.textContent;
        console.log("item", cart.item_count);
       if (cart.item_count == undefined) {
         // const abc  = JSON.parse(localStorage.getItem('___ELEVAR_GTM_SUITE--cart'));
         // let q;
         //  abc.forEach((e)=>{
         //      let vID = contextBeforeColon;
         //      if(e.variantId === vID){
         //         const q = e.quantity;
         //        console.log("q", q);
         //      }
         //  })
          setTimeout(function() {
          const qty = document.querySelector(`.cart__item[data-key="${key}"] .js-qty__wrapper input`).value;
          const qtyValue = qty-1;
           document.querySelector(`.cart__item[data-key="${key}"] .stoke-out-msg`).insertAdjacentHTML("afterbegin", `<p class="stock-out">Only ${qty} ${sizeName} available.</p>`);
          }, 1000);
        }else {
          console.log('condition not match');
          document.querySelector(`.cart__item[data-key="${key}"] .stoke-out-msg`).insertAdjacentHTML("afterbegin", `<p class="stock-out"></p>`);
        }
        return cart;
      });
    },
  ...
  // Checking if the target is the cart and the filter drawer is open. If true, close the filter.
  if (evt) {
    if (evt.target.parentElement.classList.contains("cart-link")) {
      var mobileFilterDrawerOpenedCart = document.querySelector("#FilterDrawer")
      if (mobileFilterDrawerOpenedCart && mobileFilterDrawerOpenedCart.classList.contains("drawer--is-open")) {
        mobileFilterDrawerOpenedCart.classList.add("is-transitioning");
        mobileFilterDrawerOpenedCart.classList.remove("drawer--is-open");
        window.setTimeout(function() {
          mobileFilterDrawerOpenedCart.classList.remove("is-transitioning");
        }, 500);
      }
    }
  }
  // End of checking if the target is the cart and the filter drawer is open. If true, close the filter.

  // Without this, the drawer opens, the click event bubbles up to $nodes.page, which closes the drawer.
  if (evt && evt.stopPropagation) {
    evt.stopPropagation();
    // Save the source of the click, we'll focus on this element on close
  ...
  this.bindEvents();
      ...
      close: function(evt) {
        ...
        // closing mobile nav search drawer if open
        if (evt) {
          var navSearchDrawer = document.querySelector("#NavDrawerSearch")
          var navSearchDrawerHeader = document.querySelector("#NavDrawerSearch .drawer__fixed-header ")
          if (navSearchDrawer && evt.target.classList.contains("icon-close") && navSearchDrawer.classList.contains("drawer--is-open")) {
            navSearchDrawer.classList.remove("drawer--is-open")
          }
        }
...
const secondWordBeforeSlash = splitText[0].trim().split(" ")[1];
const firstWordAfterSlash = splitText[1].trim().split(" ")[0];
const colorName = document.querySelector(".product-color-name span");
const color =  colorName.textContent;
const newText = `All ${color} in ${firstWordAfterSlash} are in your cart.`;
error.description = newText;
  ...
  theme.customerTemplates = function() {
    ...
    function resetPasswordSuccess() {
      ...
      document.body.classList.add("overflow--none");        
       ...
       window.location="https://ninetypercent.com";
       })
    ...
    CartDrawer.prototype = Object.assign({}, CartDrawer.prototype, {
      init: function() {
        ...
        document.addEventListener('ajaxProduct:added', function(evt) {
          this.cartForm.buildCart();
          // adding overlay after adding product in cart

          // end of adding overlay after adding product in cart
          if (window.innerWidth > 768){
           var cartOverlayDiv = document.querySelector(".cart-window-bg")
          if (!cartOverlayDiv.classList.contains("window-show")) {
            cartOverlayDiv.classList.add("window-show")
          }
            console.log("fromop0");
            this.open();
          }
  ...
  theme.headerNav = (function() {
    ...
    function openSearchDrawer(evt) {
      // closes the search bar if it's open
      if (document.querySelector("html").classList.contains("js-drawer-open--search")) {
        evt.preventDefault();
        closeSearchDrawer()
        return false;
      }
      ...
      // closes the header dropdown if opened
      var headerDropDowns = document.querySelectorAll(".site-nav--has-dropdown")
      if (headerDropDowns.length > 0) {
        headerDropDowns.forEach(dropDown=>{
          if (dropDown.classList.contains("show-megamenu-dropdown")) {
            dropDown.classList.remove("show-megamenu-dropdown")
          }
        })
      }
      ...
      theme.utils.prepareTransition(container, function() {
        ...
        // document.querySelector(".search-overlay").classList.add("search-overlay-appeared")
      }.bind(this));
    ...
    //HMV-24 start
    // this is for closing search drawer on clicking close button on desktop
  ...
  theme.predictiveSearch = (function() {
    ...
    if (window.innerWidth > 320) {
      ...
      selectors = {
        form: '#HeaderSearchForm_mobile',
        input: 'input[type="search"]',
        wrapper: '#PredictiveWrapper_mobile',
        resultDiv: '#PredictiveResults_mobile',
        searchButton: '[data-predictive-search-button]'
      };
    }
    ...
    function predictQuery(q) {
      ...
      .then(suggestions => {
        ...
        if (resultCount === 0) {
          cache.wrapper.classList.remove('hide');
          cache.results.innerHTML=`<p class="no-search-results">Sorry, there are no results for your query. Please try again.</p>`
          cache.submit.style.display="none"
        ...
        cache.submit.style.display="inline"
      });
    }
    ...
    function buildProducts(results) {
      ...
      results.forEach(product => {
        var alteredPrice = JSON.stringify(parseFloat(product.price)*100);
        var new_product = {
          title: product.title,
          url: product.url,
...
theme.buildProductGridItem = function(items, gridWidth, rowOf, imageSize) {
  ...
  const callback = (root) => {
    // console.log(root);
    // console.log(root.querySelectorAll('.related-products-list-container li'));
    const colorNumber = root.querySelectorAll('.related-products-list-container li').length;
    return colorNumber;
  }
  ...
  async function getAllResponse(startIndex = 0, endIndex = urlList.length) {
    const promises = [];
    for (let i = startIndex; i < endIndex; i++) {
      promises.push(getResponse(urlList[i], callback));
    }
    const allPromises = await Promise.all(promises);
    // console.log(allPromises);
    allPromises.forEach((promise,i) => {
      if(promise > 1){
        document.querySelectorAll(".recently-viewed-grid .grid-product__content .grid-product__price")[i].insertAdjacentHTML("afterend",`<div class="color-number">${promise} colors</div>`)
      }
    });
  }
  ...
  items.forEach(product => {
    ...
    const currencySymbol = document.querySelector(".footer-currency .currency-selector-toggler").innerHTML.split('(')[1][0];
    // priceElement = document.querySelector(".grid-product__price")
      ...
      <div class="grid__item grid-product ${gridWidth} aos-animate swiper-slide" data-aos="row-of-${rowOf}">
        <div class="grid-product__content">
          ...
          <div class="grid-product__meta">
            <div class="grid-product__title"><a href="${product.url}">${product.title}</a></div>
  ...
  theme.NewsletterPopup = (function () {
    function NewsletterPopup(container) {
      ...
      if (window.location.pathname === '/challenge') {
        if(window.location.href === 'https://ninetypercent.com/challenge#newsletter-footer'){
           window.location.href = "https://ninetypercent.com/challenge";
        }
        return;
      }
      ...
      if (container.querySelector('.note--success')) {
        document.querySelector(".subscribe-text").style.display = "none";
        document.querySelector(".offer-text").innerText = "THANK YOU FOR REGISTERING"
    ...
    NewsletterPopup.prototype = Object.assign({}, NewsletterPopup.prototype, {
      initPopupDelay: function () {
        ...
        setTimeout(function () {
          ...
          if (newsletterAppeared) {
            ...
            navDrawer.classList.add("is-transitioning")
          }
      ...
      closePopup: function (success) {
        ...
        var navDrawer = document.querySelector("#NavDrawer")
        document.querySelector(".drawer-menu-close.mobile-menu-header-section").click()
        // setTimeout(()=>{
        //   if(navDrawer.classList.contains("is-transitioning")) {
        //     navDrawer.classList.remove("is-transitioning")
        //   }
        // }, 50)
  ...
  theme.Collection = (function() {
    ...
    Collection.prototype = Object.assign({}, Collection.prototype, {
      ...
      setFilterStickyPosition: function() {
        var headerHeight = document.querySelector('.site-header').offsetHeight;
        var colFilterHeight = document.querySelector(".collection-grid__wrapper .collection-filter").getBoundingClientRect().height
        var mobileFilterHeight = document.querySelector(".collection-grid__wrapper .filter-sort-mobile-container").getBoundingClientRect().height
        var mainHeader = document.querySelector('[data-section-id="header"]').offsetHeight;
        var hdHeight = colFilterHeight - mobileFilterHeight - (mainHeader);
        document.querySelector(selectors.filters).style = `--topA: ${hdHeight*(-1)}px`;
...
/* currency switcher */
  ...
  DOMready(function(){
    ...
    document.dispatchEvent(new CustomEvent('page:loaded'));
     document.querySelector('[data-section-id="header"]').className = 'expand';
  });
     ...
     // swiper slider for new arrival on home page
         var swiper = new Swiper(".slider-container-swiper", {
                speed: 300,
                slidesPerView: 4.09,
                slidesPerGroup:4,
                spaceBetween: 4,
     ...
     // end of swiper slider for new arrival on home page
...
     //    prevArrow: '<div class="arrow-wrapper left"><i class="arrow-left"></div>',
    ...
    // swiper slide for mobile pdp slider
    var swiper = new Swiper(".image-wrap.pdp-mobile-slider .mySwiper", {
      pagination: {
        el: ".swiper-pagination",
        type: "fraction",
      },
...
// swiper slider for PDP page recently viewed

    var swiper9999 = new Swiper(".recently-viewed-container .mySwiper", {
                speed: 300,
                slidesPerView: 4.09,
                slidesPerGroup:4,
                spaceBetween: 4,
...
//This code is part of menu hover functionality, rest of the code is in header-desktop-nav.liquid
  $('.main-content, .announcement-bar, .header-item--logo, #shopify-section-footer, #shopify-section-icon-with-text, #shopify-section-breadcrumb-section').on('mouseenter touchstart', function() {
    if ($('.site-nav--has-dropdown').hasClass('show-megamenu-dropdown')) {
      $('.site-nav--has-dropdown').removeClass('show-megamenu-dropdown'); 
    }      
  });
...
window.addEventListener('scroll', () => {
  ...
  if (window.scrollY <=  headerHeight || currentScrollPos < prevScrollPos) {
    ...
    if(!document.querySelector('.site-header__search-container.is-active')) {
    header.classList.remove('hide');
    }
  }
...
window.addEventListener('click', (e)=>{
  if (document.querySelector(".collection-filter__item.filter-bar-btn")) {
    ...
    const collapsibleIcon = document.querySelector(".sorting-section-mobile span.collapsible-trigger-icon-sortby");
  ...
  if (e.target != mobileSortTarget) {
    ...
    collapsibleIcon.classList.remove("rotate-icon");
  }
  // targets.forEach(target=>{
  //   if (target != e.target) {
  //     var cls = target.classList[1]
  //     var filterList = document.querySelector(`.${cls} + .filter-options-list`)
  //     filterList.classList.remove("show-filter")
  //   }
  // })
  }
  // desktop search bar closing functionality
  var activeSearchBar = document.querySelector(".site-header__search-container")
  var html = document.querySelector("html")
  // var overlay = document.querySelector(".site-header__search-container .search-overlay")

  if (e.target != activeSearchBar && !activeSearchBar.contains(e.target) && html.classList.contains("js-drawer-open--search")) {
    // overlay.classList.remove("search-overlay-appeared")
    activeSearchBar.classList.add("is-transitioning")
      html.classList.add('js-drawer-closing');
      ...
      window.setTimeout(function() {
        html.classList.remove('js-drawer-closing');
        activeSearchBar.classList.remove("is-transitioning")
        html.classList.remove("js-drawer-open--search", "lock-scroll")
      }, 500);
...
var mblSearchBtns = document.querySelectorAll("#drawer-search-icon-mobile-navbar")
var searchDrawer = document.querySelector("#NavDrawerSearch")
var html = document.querySelector("html")
var body = document.querySelector("body")

mblSearchBtns.forEach(btn=>{
  btn.addEventListener('click', (e)=>{

    // start of checking if the mobile filter drawer is open. if true close it.
    var mobileFilterDrawerOpened = document.querySelector("#FilterDrawer")
    if (mobileFilterDrawerOpened) {
      if (mobileFilterDrawerOpened.classList.contains("drawer--is-open")) {
        mobileFilterDrawerOpened.classList.add("is-transitioning")
        mobileFilterDrawerOpened.classList.remove("drawer--is-open")
        window.setTimeout(function() {
          mobileFilterDrawerOpened.classList.remove("is-transitioning")
        }, 500);
      }
    }
    //end of checking if the mobile filter drawer is open. if true close it.
    
    searchDrawer.classList.add("is-transitioning")
    html.classList.add("js-drawer-open", "lock-scroll")
    searchDrawer.classList.add('drawer--is-open')
...
var searchCrossBtn = document.querySelector(".mobile-menu-search-close")
searchCrossBtn.addEventListener('click', ()=>{
    searchDrawer.classList.remove('drawer--is-open')
    html.classList.remove("js-drawer-open", "lock-scroll")
    body.classList.remove("overflow--none")
})

document.querySelector('.footer__collapsible.footer_collapsible--disabled #newsletter-footer').addEventListener("submit", () => {
  const html = document.querySelector('html');
  const popupDiv = document.querySelector('#shopify-section-np-welcome-popUp .modal.modal--square.modal--mobile-friendly.welcome-new-modal');
  
  if (html.classList.contains("modal-open")) {
    html.classList.remove("modal-open");
  }
  
  if (popupDiv.classList.contains("modal--is-active")) {
    popupDiv.classList.remove("modal--is-active");
  }
});

// PDP add to card popup for mobile and show error message when no size is selected

const addToCartBtn = document.querySelector('.page-content--product .btn.btn--full.add-to-cart');
const errorMessage = document.querySelector('#errorMessage');
const cartPopup = document.querySelector('.add-to-cart-popup');

const urlPathname = window.location.pathname;
if (urlPathname.includes('products')){
  if (urlPathname.includes('e-gift-card')) {
    addToCartBtn.addEventListener('click', function (e) {
    const selectedPrice = document.querySelector('input[name="gift card amount"]:checked');
       if (!selectedPrice) {
            e.preventDefault();
            errorMessage.innerText = 'Please select an amount';
